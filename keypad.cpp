...
void draw_keypad() {
    ...
    for (int n=1; n<10;n++) {
        screen.text(std::to_string(n), &minimal_font[0][0], pos);
        pos.x += cel;
        if (n % 3 == 0 ) {
                pos.x = center.x - cel;
                pos.y += cel;
    ...
    pos.x = center.x;
    screen.text("0", &minimal_font[0][0], pos);
}
...
Vec2 cursorpos;
    ...
    screen.text(passcode, &minimal_font[0][0], cursorpos);
    ...
    if (move.x || move.y) {
        ...
        screen.text(std::string(1,letter), &minimal_font[0][0], Vec2(0,0));
